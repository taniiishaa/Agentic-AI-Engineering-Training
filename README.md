...
```mermaid
flowchart LR
    A["🐍 Python & DSA"] --> B["⚡ Async Python"]
    B --> C["🌐 Web Development"]
    C --> D["🚀 FastAPI & Streamlit"]
    D --> E["🧠 Ollama & LLMs"]
    E --> F["🔗 LangChain"]
    F --> G["🛠️ Tools & Tool Calling"]
    G --> H["🤖 AI Agents"]
    H --> I["⚙️ Middleware"]
    I --> J["🔌 MCP / FastMCP"]
...
flowchart TD
    ...
    D -->|"Answer Directly"| E["💬 Generate Answer"]

    D -->|"Use Tool"| F["🛠️ Select Tool"]
    F --> G["⚙️ Execute"]
    G --> H["📥 Receive Result"]
    H --> B
...
flowchart TB
    ...
    UI --> LLM["🧠 Local LLM<br/>Ollama / Qwen"]

    LLM --> A["🤖 LangChain Agent"]

    A --> T{"🛠️ Select Tool"}

    T --> P["👤 Profile Tool"]
    T --> R["📦 Repository Tool"]
    T --> C["👥 Contributors Tool"]
    T --> S["📊 Statistics Tool"]

    P --> API["🐙 GitHub REST API"]
    R --> API
    C --> API
    S --> API
...
flowchart LR
    ...
    A["💻 Programming"] --> B["🌐 Applications"]
    B --> C["🧠 LLM Integration"]
    C --> D["🛠️ Tools"]
    D --> E["🤖 Agents"]
    E --> F["⚙️ Agent Workflows"]
    F --> G["🔌 MCP"]
    G --> H["🚀 Agentic Systems"]
```
